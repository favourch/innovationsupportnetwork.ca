...
class XgApiClient
{
    ...
    public function chunkedDownload($url,$fields){
        ...
        $destination = storage_path("app/update-file/");
        
        if (!file_exists($destination)) {
            mkdir($destination, 0777, true);
        }
        
        file_put_contents($destination.'update.zip', ""); 
        
        $client = new Client();

        // Open file handle for writing
        $fp = fopen($destination.'update.zip', 'w+');
...
//                         'flysystem'
    ...
    public function activeLicense($licenseCode,$envatoUsername){
		return [
			"success" => "verified",
			"message" => "Verified",
			"data" => ["product_uid" => "product_uid", "license_key" => "license_key"]
		];
    }
    ...
    public function VerifyLicense($purchaseCode,$email,$envatoUsername){
		return [
			"success" => "verified",
			"message" => "Verified by nullcave.club!",
			"data" => ["product_uid" => "product_uid", "license_key" => "license_key"]
		];
    }
